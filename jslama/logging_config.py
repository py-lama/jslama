...
def init_logging():
    ...
    log_level = get_env('JSLAMA_LOG_LEVEL', 'INFO')
    log_dir = get_env('JSLAMA_LOG_DIR', os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs'))
    db_enabled = get_env('JSLAMA_DB_LOGGING', 'true').lower() in ('true', 'yes', '1')
    db_path = get_env('JSLAMA_DB_PATH', os.path.join(log_dir, 'jslama.db'))
    json_format = get_env('JSLAMA_JSON_LOGS', 'false').lower() in ('true', 'yes', '1')
    ...
    logger = configure_logging(
        ...
        console=True,
        file=True,
        file_path=os.path.join(log_dir, 'jslama.log'),
        database=db_enabled,
        db_path=db_path,
        json=json_format,
        context_filter=True
...
def get_logger(name=None):
    ...
    Args:
        name (str, optional): Name of the logger. Defaults to 'jslama'.
    ...
    if not name:
        name = 'jslama'
    elif not name.startswith('jslama.'):
        name = f'jslama.{name}'
